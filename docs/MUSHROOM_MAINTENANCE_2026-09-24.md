...
- 只有帶 `X-Maintenance-Event: schedule` 且成功清理的專用維護請求才會更新 `mushroom-retention-scheduled` 心跳；手動 workflow 不會掩蓋排程缺席。當 Agent 上傳啟動備援，而定時心跳已逾一小時，Sites 直接送一次 Discord 警示並以 D1 原子更新限制為每小時最多一次。這是備援告警，不代表 GitHub 的定時觸發已恢復；若沒有 Agent 上傳，站台也無法主動告警。
