...
一支已 root 的 Android 手機**持續連接電腦**，電腦驅動遊戲**自動移動 GPS、對指定區域網格式掃描**，
把掃到的蘑菇（座標/等級/類型/到期時間）存起來，並透過 **Cloudflare Tunnel** 呈現在使用者**自有網域**的網頁地圖上。

- 範圍模型：**指定一塊區域（bbox）持續掃描**（非全世界——一支手機+蘑菇會過期，全世界不可行）。
- 掃到的蘑菇即時顯示在網頁地圖，過期的自動移除。
- 使用者已有 Cloudflare 網域（走 Cloudflare Tunnel，PC 一直開著）。
