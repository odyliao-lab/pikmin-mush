...
截至 2026-07-22，正式架構已由「Windows 長時間接手機掃描」改為「手機自主 Agent + 雲端集中協調」：

- 兩台實體 Android Agent 已完成並行掃描驗證；一般運作不依賴 Windows、USB、ADB、固定手機 IP 或同一區域網路。
- 雲端以逐點 lease 分派座標，同一工作內的 Agent 不會掃到相同 target；Agent 離線、停用或 lease 逾時後，座標會重新排隊。
- 每日 `07:30 Asia/Taipei` 自動換區。現有 10 天輪替表涵蓋 41 個國家／區域包、266 個主要城市配置，當日 Agent 區域互斥且城市數量盡量平衡。
...
- 中美洲：瓜地馬拉、宏都拉斯、薩爾瓦多、尼加拉瓜、哥斯大黎加、巴拿馬
- 北美洲：美國東部、美國中部、美國西部
...
- 同一天各 Agent 的國家包互不重疊；下一個 10 天週期交換分配順序。
...
3. Agent v2 API 負責逐點派工、續租與 ACK；上傳 API 解析 UTF-8 TSV 並以蘑菇 ID upsert。
...
| 路由 | 權限 | 用途 |
| --- | --- | --- |
| `GET /api/mushrooms` | 公開 | 未過期蘑菇、彙總 fleet 與粗粒度進度 |
