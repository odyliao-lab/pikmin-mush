...
| `action.sh` | Magisk 模組 Action，root | 手機端手動冷重啟 display，並恢復唯一 Agent parent。 |
...
Android boot
  ...
  -> service.sh performs bounded status probes for at most 120 seconds
  -> healthy: continue normally
  -> timeout/failure: daemon keeps recovering; Agent starts with display-0 fallback
...
所有 `dumpsys display` 與 service-level display probe 都必須由 `timeout` 包住。Android
framework 在 reboot 後可能讓 `dumpsys display` 永久阻塞；不得讓這個阻塞永遠卡住 Magisk
boot service。`service.sh` 使用 PID/cmdline 驗證的 lock 保證同時只有一個啟動流程。若
120 秒內 display 仍未 healthy，記錄 warning 後仍啟動 Agent；display daemon 繼續每
15 秒修復，Agent 在 `game.display` 無效時使用既有的 display 0 fallback。
...
`dumpsys display` 單次最多允許 8 秒，逾時視為 unhealthy 並進入下一次有界重建，不能
讓 daemon 或 boot service 永久停在 framework Binder call。
...
| Boot probe hang | 注入卡住的 `dumpsys display`；service 在有界時間內繼續啟動 Agent，daemon 可再次修復。 |
| Magisk Action | 不接 USB 執行模組 Action；舊 Agent/display 安全停止，最後只有一個有效 Agent PID 且 Fleet 恢復 online。 |
...
| Reboot 後 display 恢復但 Agent offline | 舊版 service 卡在 framework status | 在 Magisk 模組頁按 **Action**；新版會 timeout probe 並繼續啟動 Agent。 |
| Display ID 改變 | 正常重建／reboot 行為 | 不得硬編碼；以 `game.display` 為準。 |

### 16.4 手機端手動復原

Magisk 模組根目錄的 `action.sh` 是無 USB 救援入口。使用者在 Magisk 的
`Pikmin Scanner Agent` 模組按 **Action** 後，程式會：

1. 只停止 cmdline 含本模組完整路徑的 stale `service.sh`。
2. 只停止並清除本模組的 Agent PID，不依 PID 數字直接誤殺其他程序。
3. 有界停止 daemon、server、drain 與舊 display。
4. 執行新版 `service.sh`，重建 display 並啟動唯一 Agent parent。
5. 驗證 Agent PID/cmdline，將結果寫入 `manual-recovery.log`。

網站無法喚醒完全離線的 Agent，因此這個 Action、開機自動恢復及 Wi-Fi ADB 維修入口
分別構成日常、無人值守與深度診斷三層復原方式。
